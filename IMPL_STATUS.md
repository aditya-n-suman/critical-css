...
| Active package | **`apps/cli` (full)** |
| Active task | **Not started — `apps/cli` full: route manifest expansion, `--compare-baseline`, CI flags, cache wiring (BI-11)** |
| Last session | 2026-07-13 (M4: `packages/cache` complete incl. G7 fix round) |
...
**Continue M4 — next item is `apps/cli` full.** (`packages/cache` is complete.)

Per `AGENT_IMPL_BRIEF.md §Phase M4` and `docs/implementation/001-Task-Breakdown.md §8.11`:

1. **`apps/cli` full** (BI-11) — route manifest expansion (`expandRouteManifest`/`toRouteManifestEntries` from `@critical-css/cache`), `--compare-baseline` (fail build on CSS growth), CI flags, and cache wiring: `DiskCacheStore` + `CacheManager`, hot path `computeFingerprint(input).hash` → `getOrProduce(fp, runPipeline)` with browser acquisition inside the produce callback (REQ-301: a hit must provably skip browser launch). **Import `computeCacheFingerprint` from `@critical-css/cache`, NOT `@critical-css/shared`** — see Known Blockers.
2. Also open (carry-over, not blocking M4):
...
| `apps/cli` (full: routes, baseline, CI) | Not started | Cache unblocked; next up |
