...
const fixtureUrl = (name: string): string =>
	pathToFileURL(resolve(FIXTURES, name, "index.html")).href;
...
describe("packages/browser M0 integration", () => {
	...
	it("ViewportManager applies the mobile profile: window.innerWidth === 375 in-page", async () => {
		...
		try {
			...
			const width = await handle.evaluate(
				() => window.innerWidth,
				undefined as never,
			);
			...
			const navDisplay = await handle.evaluate(
				() =>
					getComputedStyle(document.getElementById("desktop-nav") as Element)
						.display,
				undefined as never,
			);
	...
	it("DOMSnapshot eagerly captures the whole reachable tree with fold + scroll context", async () => {
		...
		try {
			...
			const footer = snapshot.nodes.find(
				(n) => n.attributes["id"] === "below-fold",
			);
			expect(footer?.boundingRect.y).toBeGreaterThan(1080);
			const hidden = snapshot.nodes.find(
				(n) => n.attributes["id"] === "hidden",
			);
	...
	it("throws NavigationTimeoutError when the target is unreachable", async () => {
		...
		try {
			await expect(
				handle.navigate("http://127.0.0.1:9/unreachable", { timeoutMs: 5_000 }),
